...
from KClass.EditableTabWidget import EditableTabWidget
from KClass.CustomMenu import CustomMenu
from KClass.Dialog_Manager import TaskDialog, Dialog_Manager
from KClass.EventFilter import Filter
...
class Ui_Form(QtWidgets.QWidget):
    def __init__(self, Form):
        ...
        self.mainListView = QtWidgets.QTreeView(self.mainGroupBox)
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.MinimumExpanding,
                                           QtWidgets.QSizePolicy.MinimumExpanding)
        sizePolicy.setHorizontalStretch(0)
        sizePolicy.setVerticalStretch(0)
        ...
        self.toDoListView = QtWidgets.QTreeView(self.toDoTab)
        font = QtGui.QFont()
        font.setPointSize(9)
        ...
        #############################################
        self.memoListTab = EditableTabWidget()
        #############################################
        ...
        # model_item = QtGui.QStandardItem("aaaaaaaaaaaaaaaa")
        # model_item_1 = QtGui.QStandardItem("ccc")
        # model_item_2 = QtGui.QStandardItem("cca")
        # self.model.appendRow([model_item, model_item_1, model_item_2])
        # model_item = QtGui.QStandardItem("bbb")
        # self.model.appendRow(model_item)
        # model_item = QtGui.QStandardItem("ccc")
        # self.model.appendRow(model_item)
        self.selectListIndex = None

        self.model.setHorizontalHeaderLabels(["Task", "Stopwatch", "Target time"])
        self.toDoListView.setModel(self.model)
        self.mainListView.setModel(self.model)
        self.toDoListView.setColumnWidth(0, 400)
        self.mainListView.setColumnWidth(0, 400)
        self.toDoSelectedLineIndex = None
        ...
        self.mainListView.doubleClicked.connect(self.doubleClickedList)

        # Timer Signal and Slot
        ...
        self.addMemoButton.clicked.connect(self.addTab)
        self.removeMemoButton.clicked.connect(self.removeTab)
        self.memoListTab.tabCloseRequested.connect(self.closeTab)

        # Context Menu
        # Creates context menu automatically
        contextMenuActions = [("New", self.addListContext), ("Delete", self.deleteListContext),
                              ("Select", self.selectListItemContext),
                              ("Start", self.deleteListContext), ("Stop", self.deleteListContext),
                              ("separator", ""),
                              ("Save", self.printHello)]
        self.mainListView.customContextMenuRequested.connect(lambda pos: CustomMenu(contextMenuActions,
                                                                                    target=self.mainListView,
                                                                                    pos=pos,
                                                                                    pass_args=True))

        self.toDoListView.customContextMenuRequested.connect(lambda pos: CustomMenu(contextMenuActions,
                                                                                    target=self.toDoListView,
                                                                                    pos=pos,
                                                                                    pass_args=True))

        # Event Filter
        self._filter = Filter()
        self.mainListView.installEventFilter(self._filter)
        self._filter.left.connect(self.mainListView.clearSelection)
    ...
    def set_stopwatch(self, time):
        self.stopwatch = time
        self.timer_max = time
        self.displayStopwatchTime()

    def addListContext(self, target, pos):
        index = target.indexAt(pos).row()
        if index == -1:
            index = 0

        # Manager takes Dialog function and result method returns result when ok is pressed
        text, hour, minute, ok = Dialog_Manager(TaskDialog).result()

        # prepare target time string
        if ok:
            target_time = ""
            # hour
            target_time += ("0" + hour) if int(hour) < 10 else hour
            # minute
            target_time += ":" + ("0" + minute) if int(minute) < 10 else ":" + minute
            # second
            target_time += ":00"

            self.newListItem(text, target_time, index=index)

        if self.selectListIndex == index:
            self.selectListIndex += 1

    def newListItem(self, text, target_time, index=None):
        model_item = QtGui.QStandardItem(text)
        model_item_1 = QtGui.QStandardItem("00:00:00")
        model_item_2 = QtGui.QStandardItem(target_time)
        # Set Items
        if index is None:
            self.model.appendRow([model_item, model_item_1, model_item_2])
        else:
            self.model.insertRow(index, [model_item, model_item_1, model_item_2])

    def deleteListContext(self, target, pos):
        index = target.indexAt(pos).row()
        self.model.removeRow(index)
        print(index)

    def selectListItemContext(self, target, pos):
        index = target.indexAt(pos)
        print("YEs")
        if index.row() == -1:
            return
        self.doubleClickedList(index)
    ...
    def doubleClickedList(self, index):
        if self.selectListIndex is not None:
            item = self.model.item(self.selectListIndex)
            item.setIcon(QIcon())
        print("Yo",self.selectListIndex)
        ...
        self.selectListIndex = index.row()
        print(self.selectListIndex)
        self.set_stopwatch(QTime(1, 1, 1))
    ...
    def addTab(self):
        ...
        self.memoListTab.addTab(tab, "")
        index = self.memoListTab.indexOf(tab)
        self.memoListTab.setTabText(self.memoListTab.indexOf(tab), "Memo " + str(index + 1))

    def closeTab(self, index):
        self.memoListTab.removeTab(index)

    def removeTab(self):
        self.memoListTab.removeTab(self.memoListTab.indexOf(self.tab_8))

    def printHello(self, target, pos):
        print("Hello")
